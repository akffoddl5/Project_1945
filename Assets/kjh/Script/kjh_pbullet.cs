...
public class kjh_pbullet : MonoBehaviour
{
   ...
   static int countK=0;
    ...
    GameObject[] bulletObj=new GameObject[6];
    ...
    void bullet()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space)&&!(Input.GetKeyDown(KeyCode.Z)))
        {
            ...
            {
                bulletObj[5] = Instantiate(pbullet, pbulletL.position, pbulletL.transform.rotation);

            }
            else if (!Input.GetKey(KeyCode.Z) && Input.GetKey(KeyCode.RightArrow))
            {
                bulletObj[5] = Instantiate(pbullet, pbulletR.position, pbulletR.transform.rotation);
            }

            bulletObj[5].GetComponent<Rigidbody2D>().velocity = Vector3.forward * speed;
